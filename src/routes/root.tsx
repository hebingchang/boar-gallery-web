...
export default function Root() {
  ...
  useEffect(() => {
    axios.get<Response<string>>('https://api.gallery.boar.dyweb.sjtu.cn/geo/ip').then(async (res) => {
      if (res.data.payload === 'CN') {
        // mapbox
        axios.get<Response<string>>('https://api.gallery.boar.dyweb.sjtu.cn/mapbox/token').then((res) => {
          setToken({ type: MapType.MapBox, token: res.data.payload })
        })
      } else {
        // apple map
        axios.get<Response<string>>('https://api.gallery.boar.dyweb.sjtu.cn/mapkit-js/token').then((res) => {
            ...
            <NavbarMenu>
              ...
              <NavbarMenuItem key='lucky'>
                <Link
                  ...
                  size="lg"
                  onPress={async () => {
                    const id = (await axios.get<Response<number>>('https://api.gallery.boar.dyweb.sjtu.cn/photos/lucky')).data.payload
                    navigate(`/photo/${id}`)
                    setIsMenuOpen(false)
                  }}
                  color='foreground'
